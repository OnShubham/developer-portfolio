...
export function Socials({ size = "md" }: { size?: Size }) {
  ...
  const items = [
    profile.links.github ? { href: profile.links.github, label: "GitHub", icon: <FaGithub size={iconSize} /> } : null,
    profile.links.linkedin ? { href: profile.links.linkedin, label: "LinkedIn", icon: <FaLinkedin size={iconSize} /> } : null,
